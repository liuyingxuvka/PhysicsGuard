...
For `object_dna`, at least one current native adapter must inspect the actual governed object and emit a provider-neutral source census independent of the blueprint's selected elements, ports, semantics, cases, and mappings. Each census member has a stable source identity, kind, locator, role, content/contract fingerprint, and optional independently owned semantic fact. The reviewer keeps the full observed census even when the requested blueprint or expected-member list is smaller. It then reconciles every observed source member with exactly one explicit source-to-model mapping or evidenced terminal disposition, and checks every modeled behavior-bearing target has reverse coverage. FMI is only the first adapter that emits this contract: it discovers every archive member and every XML variable from actual bytes, then adds exact native case results and restricted oracle facts. Future adapters for other languages, tools, experiments, documents, or workflows converge on the same census contract.
...
The first implementation accepts canonical YAML/JSON target-material snapshots through one provider-neutral adapter. Target-specific adapters may follow the same capability contract later and converge on the same reviewer without adding target whitelists or caller registries.
...
The report and portable bundle carry both statuses, the source-census fingerprint, mapping coverage, and the first object-DNA gap. This keeps lightweight use cheap and honest while giving deep use an executable completion boundary.
...
The canonical transferable software-DNA bundle is materialized to an explicitly selected artifact root outside the PhysicsGuard repository only after a current static result exists. Repository-local model sources and generators remain in the denominator, but the canonical bundle, its report, indexes, and fingerprints are not repository inputs, so producing a new bundle cannot change the snapshot it fingerprints. External materialization is a separate identity-preservation gate, not an eighth understanding layer and not part of static blueprint readiness. There is no in-repository canonical copy, cache authority, compatibility export, or second current head.

At release, the bundle and a small manifest binding its fingerprint, denominator fingerprint, source commit, version, and claim boundary may be attached as dedicated release assets only after the separate test, package, installation, Git, tag, and GitHub release owners are current. Release publication and downloaded-asset verification are lifecycle gates, not static-depth evidence. The assets are not hidden inside the wheel, source distribution, installed package, or installed consumer skills. A missing, mismatched, self-contained-but-unbound, or differently sourced asset blocks the software-DNA release-asset claim without changing the earlier static result or separately proven native package behavior.
...
11. canonical out-of-repository software-DNA materialization and identity checks;
12. one full repository execution gate on a frozen source snapshot;
13. package, installation, Git/tag/GitHub release, and separately published release-asset lifecycle checks.
...
- **[Risk] Static blueprint readiness is mistaken for executed validation or release readiness.** → Keep the seven provider-neutral layers limited to static understanding; require separate execution, package, installation, Git/tag, GitHub release, materialization, and release-asset lifecycle evidence for each broader claim.
- **[Risk] Exporting the canonical software DNA into the repository makes each export change its own denominator.** → Resolve and reject in-repository output paths, export only to an external artifact root, and keep the release bundle as a separately fingerprinted release asset rather than a package or repository input.
...
11. After static readiness is derived, materialize the canonical software-DNA bundle twice to an isolated artifact root outside the repository, prove deterministic identity and zero self-fingerprinting, and freeze its lifecycle manifest without putting the bundle into the package or installed skills.
...
13. Run one final full repository validation on the frozen source snapshot, then synchronize the local package and ten installed skills, verify installation parity, update the patch version and release notes, publish the separately verified software-DNA release assets, and complete Git/tag/GitHub release closure.
