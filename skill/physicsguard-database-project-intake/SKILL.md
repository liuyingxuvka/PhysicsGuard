...
description: Use when adding, updating, admitting, staging, or registering one project inside an explicit local PhysicsGuard database. Trigger for database project intake, project admission, placeholder project, candidate project, active_registered, active_validated, or active_reusable lifecycle states.
---

# PhysicsGuard Database Project Intake

Use this route when one project should enter or update an explicit database.
...
3. Choose the lifecycle state conservatively:
   ...
   - `active_validated` only when validation evidence has been reviewed.
   - `active_reusable` only when reusable model-library evidence and limits are
     recorded.
