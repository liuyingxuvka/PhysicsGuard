...
def _run_validation_checks(
    ...
    depth_required = plan.required_checks.validation_depth or plan.claim_scope in {
        "validation_ready",
        "validated_reuse_ready",
        "prediction_ready",
    }
    if not plan.required_checks.validation and not depth_required and not plan.validation_plans:
        return
    if not plan.validation_plans:
        ...
        if depth_required:
            skipped.append(
                ProjectClosureSkippedCheck(
                    "validation_depth",
                    "no validation_plans declared, so no native depth receipt is available",
                    True,
                )
            )
        return
    for value in plan.validation_plans:
        ...
        if depth_required:
            _consume_validation_depth_receipt(
                ...
                checked_inputs,
                findings,
                requested_claim_scope=plan.claim_scope,
            )


def _consume_validation_depth_receipt(
    ...
    *,
    ...
    """Gate closure on PhysicsGuard's native receipt without recomputing residuals."""

    receipt = report.get("depth_receipt")
    if not isinstance(receipt, dict):
        findings.append(
            _finding(
                "error",
                "validation_depth_receipt_missing",
                "validation-ready closure requires a native PhysicsGuard depth receipt",
        ...
        return
    report_identity = receipt.get("report_identity") if isinstance(receipt.get("report_identity"), dict) else {}
    checked_inputs.append(
        {
            "check": "validation_depth_receipt",
            "path": str(validation_path),
            "artifact_kind": receipt.get("artifact_kind"),
            "receipt_status": receipt.get("status"),
            "requested_claim_scope": requested_claim_scope,
            "covered_scope": receipt.get("covered_scope"),
            "model_semantics": receipt.get("model_semantics"),
            "adequacy_status": (receipt.get("adequacy") or {}).get("status") if isinstance(receipt.get("adequacy"), dict) else None,
            "predictive_status": (receipt.get("predictive") or {}).get("status") if isinstance(receipt.get("predictive"), dict) else None,
            "report_sha256": report_identity.get("report_sha256"),
            "physical_recomputation": False,
        }
    )
    issues: list[str] = []
    if receipt.get("artifact_kind") != "physicsguard_validation_depth_receipt":
        issues.append("unexpected_artifact_kind")
    if receipt.get("status") != "pass" or receipt.get("ok") is not True:
        issues.append("depth_status_not_pass")
    if report_identity.get("report_type") != "model_dataset_validation":
        issues.append("report_type_mismatch")
    if report_identity.get("report_status") != report.get("status"):
        issues.append("report_status_mismatch")
    digest = report_identity.get("report_sha256")
    if not isinstance(digest, str) or len(digest) != 64:
        issues.append("report_hash_missing")
    covered_scope = receipt.get("covered_scope", receipt.get("claim_scope"))
    time_receipt = receipt.get("time") if isinstance(receipt.get("time"), dict) else {}
    adequacy = receipt.get("adequacy") if isinstance(receipt.get("adequacy"), dict) else {}
    predictive = receipt.get("predictive") if isinstance(receipt.get("predictive"), dict) else {}
    if requested_claim_scope in {"validation_ready", "validated_reuse_ready", "prediction_ready"}:
        if covered_scope == "snapshot" or time_receipt.get("snapshot_only") is True:
            issues.append("snapshot_scope_incompatible")
        if adequacy.get("status") != "pass":
            issues.append("validation_adequacy_not_pass")
    if requested_claim_scope == "prediction_ready":
        if receipt.get("model_semantics") != "stateful_dynamic":
            issues.append("stateful_dynamic_semantics_required")
        if predictive.get("status") != "pass":
            issues.append("predictive_rollout_not_pass")
        metrics = predictive.get("metrics") if isinstance(predictive.get("metrics"), dict) else {}
        if metrics.get("stability_pass") is not True:
            issues.append("predictive_stability_not_pass")
    if issues:
        findings.append(
            _finding(
                "error",
                "validation_depth_receipt_not_consumable",
                "native validation-depth receipt cannot support the requested closure claim",
                "validation_depth_receipt",
                str(validation_path),
                {
                    "issue_codes": issues,
