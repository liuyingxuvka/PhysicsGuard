...
from physicsguard.core.validation_depth import (
...
def validate_model_dataset(path: str | Path) -> ModelDatasetValidationReport:
    ...
    depth_evaluation = evaluate_validation_depth(
        plan,
        base_dir=base_dir,
        audit_spec=spec,
        scalar_observed=observed,
        scalar_direct=direct,
    )
    if depth_evaluation.series_direct_validation is not None:
        direct = dict(direct)
        direct["series_validation"] = depth_evaluation.series_direct_validation
    findings.extend(
        _finding_from_dict(item)
        for item in depth_evaluation.promoted_findings
        if item.get("type") != "physical_envelope_interval_violation"
    )

    envelope_findings = [
        *_envelope_findings(plan, observed),
        *depth_evaluation.envelope_findings,
    ]
    ...
    confidence_dicts = [item.model_dump(mode="json") for item in confidence_updates]
    finding_dicts = [asdict(item) for item in findings]
    safe_claim = _safe_claim(
        status,
        depth_status=str(depth_evaluation.payload["status"]),
        claim_scope=str(depth_evaluation.payload["claim_scope"]),
    )
    unsafe_claim_boundary = (
        "validation does not prove high-fidelity correctness, commercial-model "
        "equivalence, or validity outside the checked contract/model/data/time/scenario boundary"
    )
    ...
    summary = {
        ...
        "validation_depth_declared": plan.depth is not None,
        "validation_depth_status": depth_evaluation.payload["status"],
        "validation_claim_scope": depth_evaluation.payload["claim_scope"],
        "semantics": (
            "optimizer convergence is reported separately from validation pass; "
            "observed values are not mutated; depth receipts bind exact identities "
            "and never imply commercial-model equivalence"
        ),
    }
    hash_payload = {
        ...
        "confidence_updates": confidence_dicts,
        "findings": finding_dicts,
        "safe_claim": safe_claim,
        "unsafe_claim_boundary": unsafe_claim_boundary,
        "next_actions": next_actions,
        "summary": summary,
        "depth_receipt_without_report_identity": depth_evaluation.payload,
    }
    depth_receipt = finalize_validation_depth_receipt(
        depth_evaluation.payload,
        report_status=status,
        report_sha256=validation_outcome_sha256(hash_payload),
    )
...
def _safe_claim(status: str, *, depth_status: str, claim_scope: str) -> str:
    if status == "pass":
        if depth_status == "pass":
            return (
                "model-dataset validation and native execution-depth evidence passed "
                f"inside the exact low-fidelity {claim_scope} boundary"
            )
        return (
            "scalar model-dataset validation passed only for the checked snapshot; "
            "validation-depth evidence is incomplete and no time/scenario claim is supported"
        )
    if status == "partial":
...
def _next_actions(findings: list[ContractFinding]) -> list[str]:
    ...
    for finding in findings:
        if finding.type.startswith("validation_contract"):
            ...
            actions.append("refresh exact file hashes and rerun validation before using the depth receipt")
        elif finding.type.startswith("mapping_required_signal"):
            actions.append("resolve units, confidence, active binding, and reviewer status for required mappings")
        elif finding.type in {"snapshot_scope_overclaim", "scenario_set_scope_overclaim"}:
            actions.append("narrow the claim to snapshot scope or provide the declared time/scenario evidence")
        elif finding.type == "calibration_holdout_identity_overlap":
            actions.append("replace holdout evidence with content- and case-disjoint data")
        elif finding.type == "physical_envelope_interval_violation":
            actions.append("inspect the exact violating interval; do not hide it behind aggregate residuals")
