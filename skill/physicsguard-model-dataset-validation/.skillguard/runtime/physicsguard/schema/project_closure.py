...
class ProjectClosureRequiredChecksSpec(BaseModel):
    ...
    validation_depth: bool = False
    predictive_rollout: bool = False
...
class ProjectClosurePlanSpec(BaseModel):
    ...
    def _claim_scope_requirements(self) -> "ProjectClosurePlanSpec":
        if self.claim_scope in {"validation_ready", "validated_reuse_ready", "prediction_ready"} and not self.required_checks.validation:
            raise ValueError("validation-ready, reuse-ready, and prediction-ready closure require validation checks")
        ...
        if self.claim_scope == "prediction_ready":
            if not self.required_checks.validation_depth or not self.required_checks.predictive_rollout:
                raise ValueError("prediction_ready closure requires validation_depth and predictive_rollout checks")
        return self
