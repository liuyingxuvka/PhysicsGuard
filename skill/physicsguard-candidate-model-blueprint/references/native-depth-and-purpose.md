...
This skill's task-local owner is `physicsguard.candidate-model-blueprint` on `route:physicsguard-candidate-model-blueprint:build`; its declared closure check is `check:physicsguard-candidate-model-blueprint:task-local-model-deepening`. The shared PhysicsGuard schema and evaluator provide the envelope, while this native owner keeps the route-specific physical/evidence judgment.
