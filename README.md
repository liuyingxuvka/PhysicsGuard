...
For a database or folder that contains many PhysicsGuard projects, the database
catalog is the top-level AI map. It lists project evidence registries, generic
tags, tested quantities, model targets, validation state, model-library indexes,
and database-level gaps without copying raw datasets.
...
projects, historical tests, reusable model candidates, and missing maintenance
work, but they do not prove direct comparability. Broad cross-project claims
still require project evidence maps, validation reports, model-library checks,
and explicit comparison scope.
...
PhysicsGuard `v0.7.0` includes low-fidelity audit relations for:
...
如果一个文件夹或数据库里包含很多 PhysicsGuard 项目，数据库目录就是最高一层的 AI 地图。它登记项目证据表、通用标签、测试过的参数、模型目标、验证状态、模型库索引和数据库层缺口，但不复制原始测试数据。
...
数据库地图和查询结果只是导航输出。它可以帮助 AI 找到相关项目、历史测试、可复用模型候选和缺少维护的地方，但它不能单独证明两个项目可以直接比较。跨项目结论仍然要回到项目证据地图、验证报告、模型库检查和明确的比较范围。
...
PhysicsGuard `v0.7.0` 包含这些低保真审计关系：
